...
- [x] **歌曲批量多选管理系统（批量删除 / 批量迁移 / 批量收藏）**：在频段展开底部新增 `[多选管理]` 开关；进入多选模式后展开暗金批量控制栏（全选 / 统计 / 批量收藏 / 批量迁移 / 批量删除），曲目支持复选框与整行点击高亮，支持跨频段批量迁移与二次确认批量删除（v6.9.0）。
- [x] **全库曲目/歌手/频段即时联动搜索系统**：将顶栏连线框升级为全局搜索栏与“搜索”按钮；键入时跨频段实时过滤匹配歌名、歌手或频段名并自动展开展示，曲库无匹配时支持一键全网检索播放（v6.10.0）。
- [x] **歌单渲染 TDZ 变量初始化时序修复**：修复 `renderPlaylists` 中 `isBatchMode` 声明位置滞后导致运行时 `ReferenceError` 进而引发歌单列表空白的 bug（v6.10.1）。
- [x] **顶底功能栏结构优化（新建歌单置顶，搜索栏置底）**：将新建通讯频段（歌单）输入栏提升至面板顶部，与频段内部曲目刻录栏形成物理隔断；搜索与 AI 选歌栏下移至曲库底部，提升层次清晰度与防误触性（v6.11.0）。
- [x] **AI 选歌按钮固化置顶右上角**：将 `✨ AI选歌` 按钮常驻在顶部新建歌单按钮右侧，便于高频点击触发；底部搜索栏独占整行提供宽阔的搜索交互空间（v6.11.1）。
- [x] **频段顶部集成式图标工具栏与同列单曲刻录**：将各频段底部的重命名、多选管理、删除/清空及单曲刻录输入栏整体提升至频段展开区顶部；管理操作精简为暗金矢量图标（✏️ 重命名 / ☑️ 多选 / 🗑️ 删除），与刻录输入框和【刻录】按钮整合在单行展示，免去长列表滑动管理的繁琐路径（v6.12.0）。
- [x] **底部综合栏紧凑合并（移除冗余搜索按钮，搜索框与节点管理同行）**：利用即时搜索特质移除冗余“搜索”按钮，将全局搜索输入框与【覆盖导入 / 合并导入 / 导出 / 重置】合并为单行紧凑排布，为中间歌单列表大幅释放纵向展示空间（v6.13.0）。
- [x] **赛博等离子音频律动波形音量条上线**：彻底替代传统静态滑轨，采用 24 根独立暗金等离子柱状波形图（Equalizer Visualizer）；播放时波形柱随音乐节奏高频物理跳动，音量级别直接决定激活波形柱数量与跳动振幅上限，支持滑动/点击无级调节（v6.14.0）。
- [x] **等离子音频律动波形音量条拖拽与跨窗口事件穿透修复**：彻底修复 sandbox/iframe 中 $(window) 无法捕获 mousemove 及 touch 事件 clientX 为空导致的音量无法拖动问题；引入标准 Pointer Events 与 `setPointerCapture` 全端指针捕获，支持点击/滑动/触控无级调节、静音记忆与跨窗口穿透（v6.14.1）。
- [x] **等离子音频律动波形音量条原生滑块覆盖架构升级**：将交互层升级为原生透明 `<input type="range">` 滑块覆盖层（`.cr-vol-native-slider`），底层保持 24 根独立暗金等离子波形柱律动跳动；彻底解决各类沙箱与跨窗口环境下划动失效问题，实现 100% 极致原生顺滑拖拽与点击调节（v6.14.2）。
- [x] **Sidecar AI DJ 自定义末尾要求注入模块上线**：在设置面板 Tab 1 新增“📌 自定义末尾注入要求 (User 角色)”模块；支持输入核心选歌补充要求（如纯音乐偏好、排除人声、管弦交响、特定语言或特定氛围），在剧情正文之后、提示词最末端以 User 角色注入 payload，并支持实时字数统计与 F12 Debug 观测（v6.15.0）。

---

## 五、脚本函数职责清单 (Function Reference) — v6.15.0 现状
...
| v6.9.0 | 2026-08-26 | 歌曲批量多选管理系统上线 | 频段底部新增 `[多选管理]` 开关与暗金控制栏，支持全选/单选高亮、跨频段批量迁移、批量收藏至我的收藏及二次确认批量删除 |
| v6.10.0 | 2026-08-26 | 全库即时搜索与全网检索系统上线 | 顶部输入栏重构为“搜索栏+搜索按钮”，支持键入即时跨频段全局过滤与动态展开，曲库无果时支持一键全网连线检索 |
| v6.10.1 | 2026-08-26 | 歌单渲染 TDZ 初始化时序修复 | 修复 `renderPlaylists` 中 `isBatchMode` 声明位置滞后导致运行时 `ReferenceError` 导致歌单空白的问题 |
| v6.11.0 | 2026-08-26 | 顶底功能栏结构优化（新建歌单置顶/搜索置底） | 调整布局将新建通讯频段（歌单）提升至面板顶部，搜索栏与 AI 选歌按钮移至曲库底部，解决创建歌单与曲目刻录混淆问题 |
| v6.11.1 | 2026-08-26 | AI 选歌按钮固化顶部右上角 | 将 `✨ AI选歌` 按钮移动至顶部右上角紧邻新建按钮，底部保留纯粹搜索栏 |
| v6.12.0 | 2026-08-26 | 频段顶部集成式图标工具栏与同列刻录上线 | 频段管理按钮精简为矢量图标（✏️ 重命名 / ☑️ 多选 / 🗑️ 删除），并与刻录输入框和刻录按钮一并提至频段展开区最顶部，解决长列表滚动的痛点 |
| v6.13.0 | 2026-08-26 | 底部综合栏紧凑合并（搜索与节点管理同行） | 移除冗余搜索按钮，将搜索输入框与节点管理按钮合并排布为单行，为歌曲列表释放更多纵向可视空间 |
| v6.14.0 | 2026-08-26 | 赛博等离子音频律动波形音量条上线 | 采用 24 根独立暗金波形柱替代传统静态滑轨，支持随节拍高频跳动、音量范围高亮与手势滑动调节 |
| v6.14.1 | 2026-08-29 | 音量律动条拖拽与跨窗口事件穿透修复 | 引入 Pointer Events + setPointerCapture，修复 sandbox/iframe 跨窗体滑动失效与移动端 touch 事件 clientX 缺失问题 |
| v6.14.2 | 2026-08-29 | 等离子波形律动条原生滑块覆盖架构升级 | 采用原生透明 input[type=range] 滑块覆盖层与底层 24 根等离子波形柱协同架构，彻底保障 100% 顺滑拖动与精准点击 |
| v6.15.0 | 2026-08-29 | Sidecar AI DJ 自定义末尾要求注入模块上线 | 在设置面板中新增 User 角色末尾注入模块，支持在正文末端追加核心选歌要求，并在构建 Payload 时自动合并注入 |
